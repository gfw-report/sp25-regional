...
use retina_core::protocols::packet::{ethernet::*, ipv4::*, ipv6::*, tcp::*, Packet};
...
struct Args {
    ...
    #[clap(
        ...
        parse(from_os_str),
        value_name = "FILE",
        default_value = "header_len.jsonl"
    )]
    outfile: PathBuf,
}
...
#[derive(Serialize, Default, Debug)]
struct HeaderLenCounter {
    pub count: usize,
}

impl HeaderLenCounter {
    pub fn extend(&mut self, other: &HeaderLenCounter) {
        self.count += other.count;
    }
}

type PortResults = HashMap<u16, PortCounter>;
type HeaderLenResults = HashMap<u8, HeaderLenCounter>;
...
static TCP_HEADER_LEN_COUNTS_PKT: OnceLock<[AtomicPtr<HeaderLenResults>; ARR_LEN]> = OnceLock::new();
static TCP_HEADER_LEN_COUNTS_PKT_TLS: OnceLock<[AtomicPtr<HeaderLenResults>; ARR_LEN]> = OnceLock::new();
...
fn init_header_len_results() -> [AtomicPtr<HeaderLenResults>; ARR_LEN] {
    let mut results = vec![];
    for _ in 0..ARR_LEN {
        results.push(Box::into_raw(Box::new(HashMap::new())));
    }
...
fn tcp_header_len_counts_pkt() -> &'static [AtomicPtr<HeaderLenResults>; ARR_LEN] {
    TCP_HEADER_LEN_COUNTS_PKT.get_or_init(init_header_len_results)
}
fn tcp_header_len_counts_pkt_tls() -> &'static [AtomicPtr<HeaderLenResults>; ARR_LEN] {
    TCP_HEADER_LEN_COUNTS_PKT_TLS.get_or_init(init_header_len_results)
}
...
fn init() {
    ...
    let _ = tcp_header_len_counts_pkt();
    let _ = tcp_header_len_counts_pkt_tls();
...
// Per-packet TCP callback for header length counting
#[filter("tcp")]
fn tcp_hdl_cb(mbuf: &ZcFrame, core_id: &CoreId) {
    ...
    if let Ok(eth) = &Packet::parse_to::<Ethernet>(mbuf) {
        if let Ok(ipv4) = &Packet::parse_to::<Ipv4>(eth) {
            if let Ok(tcp) = &Packet::parse_to::<Tcp>(ipv4) {
                ...
                header_len = Some(tcp.header_len());
            }
    ...
    let ptr2 = tcp_header_len_counts_pkt()[core_id.raw() as usize].load(Ordering::Relaxed);

    let dict = unsafe { &mut *ptr };
    let dict2 = unsafe { &mut *ptr2 };
    ...
    if let Some(header_len) = header_len {
        if let Ok(header_len_u8) = header_len.try_into() {
            let entry2 = dict2.entry(header_len_u8).or_insert(HeaderLenCounter::default());
            entry2.count += 1;
        }
    }
...
// Per-packet TCP callback for header length counting
#[filter("tls")]
fn tls_hdl_cb(mbuf: &ZcFrame, core_id: &CoreId) {
    let mut header_len = None;
    if let Ok(eth) = &Packet::parse_to::<Ethernet>(mbuf) {
        if let Ok(ipv4) = &Packet::parse_to::<Ipv4>(eth) {
            if let Ok(tcp) = &Packet::parse_to::<Tcp>(ipv4) {
    ...
    let ptr2 = tcp_header_len_counts_pkt_tls()[core_id.raw() as usize].load(Ordering::Relaxed);
    let dict2 = unsafe { &mut *ptr2 };
    if let Some(header_len) = header_len {
        if let Ok(header_len_u8) = header_len.try_into() {
            let entry2 = dict2.entry(header_len_u8).or_insert(HeaderLenCounter::default());
...
struct CombinedResults {
    ...
    pub tcp_pkts_2: HeaderLenResults,
    pub tcp_pkts_3: HeaderLenResults,
...
fn combine_results(outfile: &PathBuf) {
    ...
    for core_id in 0..ARR_LEN {
        ...
        // TCP-filter header length counts
        let ptr = tcp_header_len_counts_pkt()[core_id].load(Ordering::SeqCst);
        for (key, value) in unsafe { &*ptr } {
            results
                .tcp_pkts_2
                .entry(*key)
                .or_insert(HeaderLenCounter::default())
                .extend(value);
        }

        // TLS-filter header length counts
        let ptr = tcp_header_len_counts_pkt_tls()[core_id].load(Ordering::SeqCst);
        for (key, value) in unsafe { &*ptr } {
            results
                .tcp_pkts_3
                .entry(*key)
